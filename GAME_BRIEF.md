...
- **Audio:** Procedural via the Web Audio API (`SND` object) — no sound files. The "Resonance" overhaul gives `SND` a real synthesis engine: a reusable toolkit (filtered-noise source, ADSR/percussive envelopes, filter sweeps, category buses + a shared convolver reverb), a voice orchestrator that auto-cleans nodes and caps simultaneous voices (24 desktop / 14 mobile), and a master soft-limiter. Every sound is layered + pitch-varied (combat, feedback, building/production hum, per-planet + space ambient beds). Settings expose Sound on/off, SFX Volume (low/med/high), and Planet Ambience.
